...
// Security-enhanced user defaults (removed as we expect Baserow to provide users)
const getDefaultUsers = () => []; // Return empty array as we expect users from Baserow
...
async function loadUsers() {
    try {
        let allUsers = [];
        let nextUrl = `${_config.getBase()}${_config.tables.u}/?user_field_names=true`; 
        ...
        while (nextUrl) {
            ...
            if (data.results && data.results.length > 0) {
                ...
                allUsers.push(...processedUsers); // Add processed users to the main array
        ...
        if (allUsers.length > 0) {
            ...
            showNotification(`Loaded ${allUsers.length} users from Baserow.`, 'success');
        } else {
            throw new Error('No users found from Baserow.');
        }

    } catch (error) {
        console.error('Error loading users from Baserow:', error);
        appState.users = []; // Ensure users array is empty if loading fails
        showNotification('Failed to load users from Baserow. Please check Baserow configuration.', 'error');
    }
}
...
async function saveTask() {
    ...
    try {
        ...
        const isEdit = taskId && !taskId.startsWith('demo'); // Removed demo check
...
// Removed generateDemoTasks() function
...
async function deleteTaskGroup(taskTitle) {
    ...
    try {
        ...
        for (const task of tasksToDelete) {
            // No 'demo' check needed as demo tasks are removed
